...
quaternion<T> exp2(const quaternion<T>& lhs){
    return pow(quaternion<T>(T(2), T(0), T(0), T(0)), lhs);
}
template<typename T>
...
quaternion<T> log2(const quaternion<T>& lhs){
    ...
    return quaternion<T>( T(U(ln_lhs.r) / ln_2), T(U(ln_lhs.i) / ln_2), T(U(ln_lhs.j) / ln_2), T(U(ln_lhs.k) / ln_2));
}
template<typename T>
quaternion<T> log10(const quaternion<T>& lhs){
    ...
    const auto ln_lhs = ln(lhs);
    return quaternion<T>( T(U(ln_lhs.r) / ln_10), T(U(ln_lhs.i) / ln_10), T(U(ln_lhs.j) / ln_10), T(U(ln_lhs.k) / ln_10));
}
template<typename T>
...
quaternion<T> asin(const quaternion<T>& lhs){
    const auto abs_v = sqrt(lhs.i*lhs.i + lhs.j*lhs.j + lhs.k*lhs.k);
    if(double(abs_v) < 1e-10){
        return quaternion<T>(asin(lhs.r), T(0), T(0), T(0));
    }
...
quaternion<T> acos(const quaternion<T>& lhs){
    const auto abs_v = sqrt(lhs.i*lhs.i + lhs.j*lhs.j + lhs.k*lhs.k);
    if(double(abs_v) < 1e-10){
        return quaternion<T>(acos(lhs.r), T(0), T(0), T(0));
    }
    complex<T> c(double(lhs.r), double(abs_v));
    complex<T> r = acos(c);
    const auto coeff = r.i / abs_v;
...
quaternion<T> atan(const quaternion<T>& lhs){
    const auto abs_v = sqrt(lhs.i*lhs.i + lhs.j*lhs.j + lhs.k*lhs.k);
    if(double(abs_v) < 1e-10){
        return quaternion<T>(atan(lhs.r), T(0), T(0), T(0));
    }
    complex<T> c(double(lhs.r), double(abs_v));
    complex<T> r = atan(c);
    const auto coeff = r.i / abs_v;
...
quaternion<T> atanh(const quaternion<T>& lhs){
    ...
    return quaternion<T>(ln_result.r / T(2), ln_result.i / T(2), ln_result.j / T(2), ln_result.k / T(2));
}
template<typename T>
